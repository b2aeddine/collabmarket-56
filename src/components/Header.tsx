...
const Header = () => {
  ...
  return <header className="bg-white/95 backdrop-blur-sm border-b border-gray-200 sticky top-0 z-50">
      <div className="px-4 sm:px-6 lg:px-8">
          ...
          <div className="hidden md:flex items-center space-x-3">
            ...
            {profile && <Link to="/messages">
                <Button variant="ghost" size="icon" className="relative">
                  ...
                  {unreadCount > 0 && <Badge className="absolute -top-2 -right-2 bg-red-500 text-white text-xs w-5 h-5 flex items-center justify-center p-0">
                      {unreadCount}
                    </Badge>}
                </Button>
              </Link>}
                  ...
                  <Button variant="ghost" className="flex items-center space-x-2 px-3">
                    <Avatar className="w-8 h-8">
                      ...
                      <AvatarFallback className="bg-gradient-to-r from-pink-500 to-orange-500 text-white text-sm">
                        {profile.first_name && profile.last_name ? `${profile.first_name[0]}${profile.last_name[0]}` : <User className="w-4 h-4" />}
                      </AvatarFallback>
                    </Avatar>
        ...
        {/* Mobile Navigation */}
        {isMenuOpen && <div className="md:hidden border-t border-gray-200 py-4 space-y-2">
            <Link to="/catalog" onClick={() => setIsMenuOpen(false)}>
              <Button variant={isActive('/catalog') ? 'default' : 'ghost'} className={`w-full justify-start ${isActive('/catalog') ? 'bg-gradient-to-r from-pink-500 to-orange-500 text-white' : ''}`}>
                Catalogue
              </Button>
            </Link>
            <Link to="/how-it-works" onClick={() => setIsMenuOpen(false)}>
              <Button variant={isActive('/how-it-works') ? 'default' : 'ghost'} className={`w-full justify-start ${isActive('/how-it-works') ? 'bg-gradient-to-r from-pink-500 to-orange-500 text-white' : ''}`}>
                Comment ça marche
              </Button>
            </Link>
            <Link to="/faq" onClick={() => setIsMenuOpen(false)}>
              <Button variant={isActive('/faq') ? 'default' : 'ghost'} className={`w-full justify-start ${isActive('/faq') ? 'bg-gradient-to-r from-pink-500 to-orange-500 text-white' : ''}`}>
                FAQ
              </Button>
            </Link>
            ...
            <div className="border-t border-gray-200 pt-4 space-y-2">
              ...
              {profile ? <>
                  ...
                  <Link to="/signup" onClick={() => setIsMenuOpen(false)}>
                    <Button className="w-full bg-gradient-to-r from-pink-500 to-orange-500 hover:opacity-90">
                      Inscription
                    </Button>
                  </Link>
                </>}

...
const EditProfileModal = ({ user, onSave, isOpen, onClose }: EditProfileModalProps) => {
  const { updateProfile, user: currentUser } = useAuth();
  const { categories } = useCategories();
  const { profileCategories, refetch: refetchProfileCategories } = useProfileCategories(user.id);
            ...
            <div className="flex flex-wrap gap-2 min-h-[60px] p-3 border rounded-md">
              {selectedCategories.length === 0 ? (
                ...
                selectedCategories.map((categoryId) => {
                  ...
                  return (
                    <Badge key={categoryId} variant="default" className="flex items-center gap-1 bg-gradient-to-r from-pink-500 to-orange-500 text-white">
                      {category?.name}
                      <X 
